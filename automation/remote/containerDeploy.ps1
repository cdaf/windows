...
function executeExpression ($expression) {
	...
	try {
		...
		if (( $LASTEXITCODE ) -and ( $LASTEXITCODE -ne 0 )) {
			ERRMSG "[DC][EXCEPTION] $message" $LASTEXITCODE
		} else {
			ERRMSG "[DC][EXCEPTION] $message" 1212
		}
	}
    if ( $LASTEXITCODE ) {
    	if ( $LASTEXITCODE -ne 0 ) {
			ERRMSG "[DC][EXIT] `$LASTEXITCODE is $LASTEXITCODE" $LASTEXITCODE
		} else {
			if ( $error ) {
				ERRMSG "[DC][WARN] `$LASTEXITCODE is $LASTEXITCODE, but standard error populated"
			}
		} 
	} else {
	    if ( $error ) {
	    	if ( $env:CDAF_IGNORE_WARNING -eq 'no' ) {
				ERRMSG "[DC][ERROR] `$env:CDAF_IGNORE_WARNING is 'no' so exiting" 1213
	    	} else {
				ERRMSG "[DC][WARN] `$LASTEXITCODE not set, but standard error populated"
	    	}
		}
	}
}
...
foreach ( $envVar in Get-ChildItem env:) {
	if ($envVar.Name.Contains('CDAF_OPT_')) {
		$buildCommand += ' ' + $envVar.Value
	}
}
